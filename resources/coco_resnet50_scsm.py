...
with open('/home/charan/Documents/workspaces/python_workspaces/python_practice_workspace/coco_tools/image_ids_new.txt',
          'r') as f:
    imglist = f.readlines()
    f.close()

imglist = [os.path.join(img_directory, each.strip()) for each in imglist]
...
def inference(threshold):
    ...
    artifact_path = 'Test-Artifacts/' + backbone + '/SCSM-500/' + str(threshold) + '/'
    output_path = artifact_path + 'output/'
    ...
    for index, filename in enumerate(tqdm(imglist)):
        ...
        for idx, cls in enumerate(tqdm(CLASSES)):
            ...
            net.reset_class(classes=[cls], reuse_weights=[cls])
